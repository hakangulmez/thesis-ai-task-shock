...
These descriptive comparisons use raw revenue levels and are therefore sensitive to the size composition of each quartile — larger firms within a quartile exert disproportionate influence on the group mean. The DiD regression addresses this concern by log-scaling revenue and including firm fixed effects, which absorb all time-invariant size differences and identify the treatment effect from within-firm variation over time.
